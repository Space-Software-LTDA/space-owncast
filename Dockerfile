...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    CGO_ENABLED=1 GOOS=linux go build -a -installsuffix cgo -ldflags "-extldflags \"-static\" -s -w -X github.com/Space-Software-LTDA/owncast/config.GitCommit=$GIT_COMMIT -X github.com/Space-Software-LTDA/owncast/config.VersionNumber=$VERSION -X github.com/Space-Software-LTDA/owncast/config.BuildPlatform=$NAME" -o owncast .
