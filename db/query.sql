...
INSERT INTO ap_outbox(iri, value, type, live_notification)
values($1, $2, $3, $4);
-- name: AddToAcceptedActivities :exec
INSERT INTO ap_accepted_activities(iri, actor, type, timestamp)
values($1, $2, $3, $4);
-- name: GetInboundActivityCount :one
SELECT count(*)
FROM ap_accepted_activities;
...
INSERT INTO ip_bans(ip_address, notes)
values($1, $2);
...
INSERT INTO notifications (channel, destination)
VALUES($1, $2);
...
INSERT INTO auth(user_id, token, type)
values($1, $2, $3);
...
WHERE token = $1
	AND auth.type = $2
	AND users.id = auth.user_id;
-- name: AddAccessTokenForUser :exec
INSERT INTO user_access_tokens(token, user_id)
values($1, $2);
...
UPDATE users
SET display_name = $1,
	previous_names = previous_names || $2,
	namechanged_at = $3
WHERE id = $4;
